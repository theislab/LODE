...
full_pred_paths = [os.path.join(prediction_path,i) for i in os.listdir(prediction_path)]
...
for count, pp in tqdm(enumerate(full_pred_paths)):
    try:
        save_name = "low_res_" + pp.split("/")[-1]
        prediction = np.load(pp)
        full_size_pred = cv2.resize(prediction[0,:,:,0], (768,768), cv2.INTER_NEAREST)
...
low_res_pd.to_csv("pred_etdrs_max_values.csv")