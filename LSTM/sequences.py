...
def split_sequences(sequences, min_len=None, max_len=None, train_frac=0.8, val_frac=0.1, num_train=None, diagnosis=None, seed=42, log=None, only_hard_sequences=False):
    ...
    if only_hard_sequences is True:
        ...
        cur_sequences = [seq for seq in cur_sequences if np.argmax(IOVar.CHECKUP1_DIFF_VA_CLASS.get_data_from_mmt(seq.measurements[-1])) != 2]
            ...
            for checkup in ('checkup1', 'checkup2'):
                ...
                if checkup_mmt is not None:
                    try:
                        ...
                        if normalize == 'retina':
                            # normalize by retinal features
                            checkup_mmt.features = checkup_mmt.features / checkup_mmt.features[[1,4,5,6,7,8,9,10,11]].sum()
                    except KeyError as e:
