...
def percentual_deviance(label,prediction):
...
def write_depthgrid_values(coord_list,value_list,text_size):
...
def plot_fundus(label_path, image_path, save_name,save_path,laterality):
    #center image
...
def plot_fundus_label_and_prediction(label_path,prediction_path, image_path,
    ...
    prediction_mu = cv2.resize(np.load(os.path.join(prediction_path,
                                                    save_name.replace(".png",".npy"))).reshape(1,256,256,1)[0,:,:,0],(768,768))*500.
    ...
    prediction_mu = center_img(prediction_mu)

    percentual_dev = percentual_deviance(label_mu, prediction_mu)
    #load image and set margin to zero and center
    ...
    low_res_grid_prediction = get_low_res_grid_shape(prediction_mu)
    ...
    plt.suptitle(title_text.format(answers["referral_answer_f"],answers["a_e_n_answer_f"],
                                   ...
                                   answers["gold_standard_referral"], answers["gold_standard_a_e_n"]),size=sup_text_size)
    ...
    write_depthgrid_values(coord_list, low_grid_values_label,text_size-10)
    plt.colorbar(fraction=0.046, pad=0.04).ax.tick_params(labelsize=text_size*0.8)
    ...
    plt.title("laterality: {}".format(laterality),size=text_size)
    write_depthgrid_values(coord_list, low_grid_values_prediction,text_size-10)
    plt.colorbar(fraction=0.046, pad=0.04).ax.tick_params(labelsize=text_size*0.8)

    plt.savefig(os.path.join(save_path, str(save_name)))
    plt.close()

def heidelberg_colormap():
