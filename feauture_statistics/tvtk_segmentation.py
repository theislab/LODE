...
DATA_PATH = PROJ_DIR + RECORD_PATH
# PNG files path source (stack of images)
files=glob.glob(DATA_PATH+"/vol/*.png")

octs = np.zeros([len(files), 128, 128]).astype("float")
for k, file in enumerate(files):
    oct = cv2.resize(cv2.imread(file, 0)/ 255., (128, 128))
    octs[k, :, :] = oct
...
surf = mlab.pipeline.surface(compute_normals, color=(0.5, 0.5, 0.5))
