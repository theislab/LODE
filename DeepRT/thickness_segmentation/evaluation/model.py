...
def get_shallow_bunet(input_img, n_filters=16, dropout=1.0, batchnorm=True, training =True):
    # contracting path
    c1 = conv2d_block(input_img, n_filters=n_filters * 1, kernel_size=3, batchnorm=batchnorm)
    p1 = MaxPooling2D((2, 2))(c1)
    p1 = Dropout(dropout)(p1, training=training)
    ...
    # expansive path
    u8 = Conv2DTranspose(n_filters *16, (3, 3), strides=(2, 2), padding='same')(c6)
    u8 = concatenate([u8, c5])
    u8 = Dropout(dropout)(u8, training=training)
    c9 = conv2d_block(u8, n_filters=n_filters * 16, kernel_size=3, batchnorm=batchnorm)
...
def get_very_shallow_bunet(input_img, n_filters=16, dropout=1.0, batchnorm=True, training =True):
    # contracting path
    c1 = conv2d_block(input_img, n_filters=n_filters * 1, kernel_size=3, batchnorm=batchnorm)
    p1 = MaxPooling2D((2, 2))(c1)
    p1 = Dropout(dropout)(p1, training=training)
    ...
    c6 = conv2d_block(p4, n_filters=n_filters * 32, kernel_size=3, batchnorm=batchnorm)

    # expansive path
    u9 = Conv2DTranspose(n_filters *8, (3, 3), strides=(2, 2), padding='same')(c6)
    u9 = concatenate([u9, c4])
    u9 = Dropout(dropout)(u9, training=training)
    c10 = conv2d_block(u9, n_filters=n_filters * 8, kernel_size=3, batchnorm=batchnorm)
